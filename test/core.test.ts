...
const knownMnemonic =
  'rich banner swift brush fury tunnel pause forest dose color luggage length';
...
const knownStdSignature = {
  signature:
    'zbaibf4Dh4wwM0spbZlnUWR9mGN8HwFUqyp29Mf7Ysoa0iVKUZuXrYAfTNP7pmwhMmAgp/3dolIiitQVt9tQIw==',
  pub_key: {
...
describe('core', () => {
  describe('createWalletFromMnemonic', () => {
    ...
    it('with default password, custom prefix, and custom path', () => {
      const wallet = Sig.createWalletFromMnemonic(
        knownMnemonic,
        undefined,
        customPrefix,
        customPath
  ...
  describe('createWalletFromMasterKey', () => {
    ...
    it('with custom prefix and path', () => {
      const wallet = Sig.createWalletFromMasterKey(
        knownMasterKey,
        customPrefix,
        customPath
      );
  ...
  describe('createSignatureBytes', () => {
    it('with signMsg and privateKey', () => {
      const signature = Sig.createSignatureBytes(
        knownSignMsg,
        cosmosPrivateKey
      );
  ...
  describe('verifySignatureBytes', () => {
    it('with signMsg, signature, and publicKey', () => {
      const valid = Sig.verifySignatureBytes(
        knownSignMsg,
        knownSignature,
        cosmosPublicKey
      );
